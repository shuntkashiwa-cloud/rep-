...
export default function TimeUI({settime,type}:Props) {
  ...
  return (
    <div>
      ...
      }} onFocus={() => { setFocused([true, false]); count.current[0] = 0; }} onBlur={() => {settime(hour[0]*60+hour[1]);setFocused((prev) => { const buf = [...prev]; buf[0] = false; return buf; })}}>{focused[0] ? <mark className="p-px bg-[oklch(0.54_0.17_262.17)] text-white">{(hour[0]).toString().padStart(2, '0')}</mark> :(hour[0]+24%24).toString().padStart(2, '0')}</p>
      <div className={`relative ${type? "bg-white":"bg-gray-100"} border border-gray-300 rounded mt-1 shadow-lg h-24 w-[161px]`}>
        {["32", "79"].map((i, inde) => (
